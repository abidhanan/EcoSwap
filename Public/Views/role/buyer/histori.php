...
// A. Konfirmasi Pesanan Diterima
if (isset($_POST['action']) && $_POST['action'] == 'confirm_received') {
    $oid = $_POST['order_id'];
    
    // Ambil Data Toko & Invoice
...
if (isset($_POST['action']) && $_POST['action'] == 'report_seller') {
    ...
    if (mysqli_query($koneksi, $query_report)) {
        echo "<script>alert('Laporan berhasil dikirim. Admin akan meninjau bukti Anda.'); window.location.href='histori.php';</script>";
    } else {
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    // --- LOGIKA HITUNG TOTAL (PRODUK + ONGKIR + ADMIN) ---
    
    // 1. Parsing String Pengiriman untuk ambil harga ongkir
    // Format di DB: "JNE (Rp 15.000) | Dana"
    $parts = explode(' | ', $row['shipping_method']);
    $ship_lbl_full = $parts[0]; // "JNE (Rp 15.000)"
    $pay_lbl = isset($parts[1]) ? $parts[1] : 'Manual/COD';
    
    $shipping_cost = 0;
    // Regex untuk ambil angka di dalam (Rp ...)
    ...
    $history_data[] = [
        ...
        // Harga satuan di list utama (opsional mau grand total atau harga barang)
