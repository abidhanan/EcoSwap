...
if (isset($_POST['action']) && $_POST['action'] == 'report_chat') {
    ...
    $admin_id = $d_adm['user_id'];

    // Kirim Notifikasi ke Admin sebagai Laporan (Menghindari error FK di tabel reports order)
...
// ... (Handler create_order, mark_read, dll TETAP SAMA seperti sebelumnya) ...
...
if (isset($_POST['action']) && $_POST['action'] == 'send_message') { header('Content-Type: application/json'); $receiver_id = $_POST['receiver_id']; $message = mysqli_real_escape_string($koneksi, $_POST['message']); if(!empty($message) && !empty($receiver_id)) { $insert = mysqli_query($koneksi, "INSERT INTO chats (sender_id, receiver_id, message, created_at) VALUES ('$user_id', '$receiver_id', '$message', NOW())"); if($insert) echo json_encode(['status' => 'success']); else echo json_encode(['status' => 'error']); } else { echo json_encode(['status' => 'empty']); } exit; }
if (isset($_GET['action']) && $_GET['action'] == 'filter_products') { header('Content-Type: application/json'); $category_filter = isset($_GET['category']) ? $_GET['category'] : 'Semua'; $where_clause = "WHERE p.status = 'active'"; if ($category_filter != 'Semua') { $safe_cat = mysqli_real_escape_string($koneksi, $category_filter); if ($safe_cat == 'Fashion') $where_clause .= " AND (p.category = 'Fashion Pria' OR p.category = 'Fashion Wanita')"; elseif ($safe_cat == 'Hobi') $where_clause .= " AND p.category = 'Hobi & Koleksi'"; else $where_clause .= " AND p.category = '$safe_cat'"; } $query_prod = mysqli_query($koneksi, "SELECT p.*, s.shop_name, s.shop_image, s.shop_id, s.user_id as seller_id, s.shop_city, a.full_address FROM products p JOIN shops s ON p.shop_id = s.shop_id LEFT JOIN addresses a ON s.user_id = a.user_id AND a.is_primary = 1 $where_clause ORDER BY p.created_at DESC"); $filtered_products = []; while($row = mysqli_fetch_assoc($query_prod)) { $shop_id_prod = $row['shop_id']; $is_following = false; $q_check = mysqli_query($koneksi, "SELECT 1 FROM shop_followers WHERE shop_id='$shop_id_prod' AND user_id='$user_id'"); if($q_check && mysqli_num_rows($q_check) > 0) $is_following = true; $city = !empty($row['shop_city']) ? $row['shop_city'] : (isset($row['full_address']) ? trim(explode(',', $row['full_address'])[2] ?? 'Indonesia') : 'Indonesia'); $city = str_replace(['Kota ', 'Kabupaten '], '', $city); $img_db = $row['image']; if (strpos($img_db, 'http') !== 0) { $img_db = str_replace('../../../', '../../../', $img_db); } $filtered_products[] = [ 'id' => $row['product_id'], 'title' => $row['name'], 'price' => (int)$row['price'], 'loc' => $city, 'img' => $img_db, 'cond' => $row['condition'], 'desc' => $row['description'], 'category' => $row['category'], 'shop_name' => $row['shop_name'], 'shop_img' => $row['shop_image'], 'shop_id' => $row['shop_id'], 'seller_id' => $row['seller_id'], 'shop_address' => $city, 'is_following' => $is_following ]; } echo json_encode($filtered_products); exit; }
...
$addresses = []; $q_addr = mysqli_query($koneksi, "SELECT * FROM addresses WHERE user_id = '$user_id' ORDER BY is_primary DESC"); while($row = mysqli_fetch_assoc($q_addr)){ $details = []; if(!empty($row['village'])) $details[] = "Kel. " . $row['village']; if(!empty($row['subdistrict'])) $details[] = "Kec. " . $row['subdistrict']; if(!empty($row['city'])) $details[] = $row['city']; if(!empty($row['postal_code'])) $details[] = $row['postal_code']; $row['formatted_details'] = implode(", ", $details); $addresses[] = $row; } $default_addr = !empty($addresses) ? $addresses[0] : null;

// --- PERBAIKAN QUERY CHAT: AMBIL FOTO PROFIL (s_pic, r_pic) ---
$chat_partners = []; $chat_messages_grouped = []; 
$q_chat = mysqli_query($koneksi, "
    SELECT c.*, 
           sender.user_id as s_id, sender.name as s_name, sender.profile_picture as s_pic,
           receiver.user_id as r_id, receiver.name as r_name, receiver.profile_picture as r_pic
    FROM chats c 
...
while($row = mysqli_fetch_assoc($q_chat)){ 
    if($row['sender_id'] == $user_id){ 
        $pid = $row['receiver_id']; $pname = $row['r_name']; $ppic = $row['r_pic']; $type = 'outgoing'; 
    } else { 
        $pid = $row['sender_id']; $pname = $row['s_name']; $ppic = $row['s_pic']; $type = 'incoming'; 
    } 
    ...
    if(!isset($chat_partners[$pid])) { $chat_partners[$pid] = ['id'=>$pid, 'name'=>$pname, 'img'=>$ppic, 'last_msg'=>$row['message'], 'time'=>date('H:i', strtotime($row['created_at']))]; } 
...
<head>
    ...
    <style>
        ...
        .chat-context-menu {
            ...
            z-index: 10000; min-width: 120px; display: none;
        }
        .chat-context-item {
...
<body>
    ...
    <div class="chat-sidebar" id="chatSidebar">
        ...
        <div class="chat-items" id="chatItemsContainer">
            ...
            <?php else: ?>
                ...
                <div class="chat-item" onclick="selectChat(<?php echo $pid; ?>, '<?php echo addslashes($info['name']); ?>', '<?php echo $info['img']; ?>')">
    ...
    <script>
        ...
        // ... (Fungsi renderProducts, filterCategory, Modal SAMA) ...
        ...
        function openModal(product) {
            ...
            const btnChat = document.getElementById('btnModalChat'); const newBtnChat = btnChat.cloneNode(true); btnChat.parentNode.replaceChild(newBtnChat, btnChat); newBtnChat.onclick = function() { closeModal(); toggleChat(); selectChat(product.seller_id, product.shop_name, shopImg); };
        ...
        // ... (Logic Checkout, Cart, Follow SAMA) ...
        function toggleFollow(shopId, btn, shopName) { btn.disabled = true; btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Loading...'; const formData = new FormData(); formData.append('action', 'toggle_follow'); formData.append('shop_id', shopId); fetch('dashboard.php', { method: 'POST', body: formData }).then(res => res.json()).then(data => { if (data.status === 'followed') { btn.classList.add('following'); btn.innerHTML = '<i class="fas fa-check"></i> Mengikuti'; updateLocalProductFollowStatus(shopId, true); } else if (data.status === 'unfollowed') { btn.classList.remove('following'); btn.innerHTML = '<i class="fas fa-plus"></i> Ikuti'; updateLocalProductFollowStatus(shopId, false); } }).finally(() => { btn.disabled = false; }); }
        function updateLocalProductFollowStatus(shopId, status) { products.forEach(p => { if(p.shop_id == shopId) { p.is_following = status; } }); }
        function addToCart() { if(!currentActiveProduct) return; const btn = document.getElementById('btnAddToCart'); const originalText = btn.innerHTML; btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Menambahkan...'; btn.disabled = true; const formData = new FormData(); formData.append('action', 'add_to_cart'); formData.append('product_id', currentActiveProduct.id); fetch('dashboard.php', { method: 'POST', body: formData }).then(res => res.json()).then(data => { if(data.status === 'success') { const badge = document.getElementById('navCartBadge'); badge.style.display = 'block'; badge.textContent = data.new_count; const container = document.getElementById('cartItemsContainer'); if(container.querySelector('div') && container.querySelector('div').textContent === 'Keranjang kosong') container.innerHTML = ''; const newItem = document.createElement('div'); newItem.className = 'cart-item'; newItem.onclick = function() { toggleCartItem(this); }; newItem.dataset.id = data.cart_id; newItem.dataset.shopId = currentActiveProduct.shop_id; newItem.dataset.productId = currentActiveProduct.id; newItem.dataset.price = currentActiveProduct.price; newItem.dataset.name = currentActiveProduct.title; newItem.dataset.img = currentActiveProduct.img; newItem.style.cursor = 'pointer'; newItem.innerHTML = `<div class="cart-check-wrapper"><input type="checkbox" class="cart-checkbox" onclick="event.stopPropagation(); updateCartTotal()"></div><img src="${currentActiveProduct.img}" class="cart-item-img" alt="Item"><div class="cart-item-info"><div class="cart-item-title">${currentActiveProduct.title}</div><div class="cart-item-price">Rp ${currentActiveProduct.price.toLocaleString('id-ID')}</div></div><button class="btn-delete" onclick="deleteCartItem(event, ${data.cart_id})"><i class="fas fa-trash"></i></button>`; container.prepend(newItem); closeModal(); setTimeout(toggleCart, 300); alert("Barang berhasil ditambahkan ke keranjang!"); } else { alert("Gagal menambahkan: " + data.message); } }).finally(() => { btn.innerHTML = originalText; btn.disabled = false; }); }
        function deleteCartItem(e, cartId) { e.stopPropagation(); if(!confirm("Hapus barang ini dari keranjang?")) return; const itemElement = document.querySelector(`.cart-item[data-id='${cartId}']`); const formData = new FormData(); formData.append('action', 'delete_item'); formData.append('cart_id', cartId); fetch('dashboard.php', { method: 'POST', body: formData }).then(res => res.json()).then(data => { if(data.status === 'success') { if(itemElement) itemElement.remove(); const badge = document.getElementById('navCartBadge'); if(data.new_count > 0) { badge.textContent = data.new_count; } else { badge.style.display = 'none'; document.getElementById('cartItemsContainer').innerHTML = '<div style="text-align:center; padding:20px; color:#666;">Keranjang kosong</div>'; } updateCartTotal(); } else { alert("Gagal menghapus barang."); } }); }
        
        // Checkout functions...
        const formatRupiah = (n) => new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(n);
        let checkoutProductPriceTotal = 0; const checkoutAdminFee = <?php echo $admin_fee; ?>; 
        let activePaymentCategory = null; let activePaymentName = ''; let selectedShippingCost = 0; let selectedShippingName = ''; const courierPrices = { "COD": 0, "JNE Reguler": 15000, "J&T Express": 18000, "SiCepat": 12000, "GoSend Instant": 25000, "GrabExpress": 24000, "AnterAja": 11000 };
        function checkoutFromCart() { const items = []; let shopId = null; document.querySelectorAll('.cart-item').forEach(el => { if(el.querySelector('.cart-checkbox').checked) { items.push({ title: el.getAttribute('data-name'), price: parseInt(el.getAttribute('data-price')), img: el.getAttribute('data-img'), id: el.getAttribute('data-product-id'), shop_id: el.getAttribute('data-shop-id'), cart_id: el.getAttribute('data-id') }); shopId = el.getAttribute('data-shop-id'); } }); if(items.length===0) { alert("Pilih minimal satu barang."); return; } localStorage.setItem('checkoutItems', JSON.stringify(items)); localStorage.setItem('checkoutShopId', shopId); toggleCart(); initCheckoutModal(); }
        function buyNow() { if(!currentActiveProduct) return; localStorage.setItem('checkoutItems', JSON.stringify([{ title: currentActiveProduct.title, price: currentActiveProduct.price, img: currentActiveProduct.img, id: currentActiveProduct.id, shop_id: currentActiveProduct.shop_id }])); localStorage.setItem('checkoutShopId', currentActiveProduct.shop_id); closeModal(); initCheckoutModal(); }
        async function initCheckoutModal() { document.body.classList.add('no-scroll'); const storedData = localStorage.getItem('checkoutItems'); const shopId = localStorage.getItem('checkoutShopId'); const container = document.getElementById('checkoutProductList'); container.innerHTML = ''; checkoutProductPriceTotal = 0; selectedShippingCost = 0; selectedShippingName = ''; activePaymentCategory = null; activePaymentName = ''; if (storedData) { const data = JSON.parse(storedData); data.forEach(item => { checkoutProductPriceTotal += item.price; container.innerHTML += `<div class="product-row-checkout"><img src="${item.img}" class="product-img-checkout"><div class="product-details-checkout"><div class="prod-name-checkout">${item.title}</div><div class="prod-price-checkout">${formatRupiah(item.price)}</div></div></div>`; }); } document.getElementById('shippingContainer').innerHTML = '<div style="text-align:center; padding:10px; color:#888;">Memuat opsi pengiriman...</div>'; document.getElementById('paymentContainer').innerHTML = '<div style="text-align:center; padding:10px; color:#888;">Memuat metode pembayaran...</div>'; calculateCheckoutTotal(); document.getElementById('checkoutModal').classList.add('open'); if(shopId) { try { const fd = new FormData(); fd.append('action', 'get_shop_settings'); fd.append('shop_id', shopId); const response = await fetch('dashboard.php', { method: 'POST', body: fd }); const settings = await response.json(); renderShippingOptions(settings.shipping); renderPaymentMethods(settings.payment); } catch (e) { console.error(e); } } }
        function renderShippingOptions(options) { const container = document.getElementById('shippingContainer'); container.innerHTML = ''; if(options && options.length > 0) { options.forEach(opt => { const price = courierPrices[opt] !== undefined ? courierPrices[opt] : 15000; container.innerHTML += `<div class="shipping-option-card" onclick="selectShippingOption(this, '${opt}', ${price})"><div class="ship-info"><span class="ship-name">${opt}</span><span class="ship-price">${formatRupiah(price)}</span></div><i class="fas fa-check-circle check-circle" style="display:none; color:var(--primary);"></i></div>`; }); } else { container.innerHTML = '<div style="color:#888; padding:10px;">Toko belum mengatur pengiriman.</div>'; } }
        function selectShippingOption(el, name, price) { document.querySelectorAll('.shipping-option-card').forEach(c => { c.classList.remove('active'); c.querySelector('.check-circle').style.display = 'none'; }); el.classList.add('active'); el.querySelector('.check-circle').style.display = 'block'; selectedShippingCost = price; selectedShippingName = name; calculateCheckoutTotal(); }
        function renderPaymentMethods(methods) { const container = document.getElementById('paymentContainer'); container.innerHTML = ''; const cats = { 'Transfer Bank': {id: 'bank', icon: 'fa-university', items: []}, 'E-Wallet': {id: 'ewallet', icon: 'fa-wallet', items: []}, 'COD': {id: 'cod', icon: 'fa-hand-holding-usd', items: []} }; if(methods) { methods.forEach(pay => { if(pay.includes('Bank') || pay.includes('BCA') || pay.includes('BRI') || pay.includes('Mandiri')) cats['Transfer Bank'].items.push(pay); else if(pay.includes('Pay') || pay.includes('OVO') || pay.includes('Dana')) cats['E-Wallet'].items.push(pay); else if(pay.includes('COD')) cats['COD'].items.push(pay); }); } for (const [key, cat] of Object.entries(cats)) { if(cat.items.length > 0) { let subHTML = ''; cat.items.forEach(i => { subHTML += `<div class="sub-option" onclick="selectPaymentSubOption('${cat.id}', '${i}', '${i}')"><i class="far fa-circle" style="font-size:0.8rem; margin-right:8px; color:#bbb;"></i> ${i}</div>`; }); const dropIcon = (key !== 'COD') ? `<i class="fas fa-chevron-down drop-icon"></i>` : ''; const toggleAction = (key !== 'COD') ? `onclick="togglePaymentDropdown(event, 'list-${cat.id}')"` : `onclick="selectPaymentCategory('cod', 'COD (Bayar di Tempat)')"`; const listDisplay = (key !== 'COD') ? `<div class="payment-options-list" id="list-${cat.id}">${subHTML}</div>` : ''; container.innerHTML += `<div class="payment-category" id="cat-${cat.id}" ${toggleAction}><div class="payment-header"><div class="ph-left"><i class="fas ${cat.icon}" style="width:20px; text-align:center; margin-right:8px;"></i><span class="ph-title">${key}</span></div><div class="ph-right">${dropIcon}<i class="fas fa-check-circle check-circle" id="check-${cat.id}" style="display:none; color:var(--primary);"></i></div></div>${listDisplay}</div>`; } } if(container.innerHTML === '') container.innerHTML = '<div style="color:#888; padding:10px;">Toko belum mengatur pembayaran.</div>'; }
        ...
        function calculateCheckoutTotal() { const total = checkoutProductPriceTotal + selectedShippingCost + checkoutAdminFee; document.getElementById('summaryProdPrice').innerText = formatRupiah(checkoutProductPriceTotal); document.getElementById('summaryShipPrice').innerText = formatRupiah(selectedShippingCost); document.getElementById('summaryTotal').innerText = formatRupiah(total); document.getElementById('bottomTotal').innerText = formatRupiah(total); }
        ...
        function processOrder() { if (!selectedShippingName) { alert("Pilih pengiriman."); return; } if (!activePaymentCategory) { alert("Pilih metode pembayaran."); return; } if (!selectedAddressId) { alert("Pilih alamat pengiriman."); return; } const items = localStorage.getItem('checkoutItems'); if(confirm("Buat pesanan sekarang?")) { const btn = document.querySelector('.btn-order'); btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Memproses...'; btn.disabled = true; const fd = new FormData(); fd.append('action', 'create_order'); fd.append('address_id', selectedAddressId); fd.append('shipping_method', selectedShippingName); fd.append('shipping_cost', selectedShippingCost); fd.append('payment_method', activePaymentName); fd.append('items', items); fetch('dashboard.php', { method: 'POST', body: fd }).then(res => res.json()).then(data => { if(data.status === 'success') { alert("Pesanan berhasil dibuat!"); closeCheckoutModal(); window.location.href = 'histori.php'; } else { alert("Gagal: " + data.message); btn.innerHTML = 'Buat Pesanan'; btn.disabled = false; } }).catch(err => { console.error(err); alert("Error sistem."); btn.innerHTML = 'Buat Pesanan'; btn.disabled = false; }); } }
        ...
        function selectAddress(el, name, detail, phone) { const displayBox = document.querySelector('.address-box'); displayBox.innerHTML = `<div class="addr-header-row"><span class="addr-recipient">${name}</span><span class="addr-divider">|</span><span class="addr-phone">${phone}</span></div><div class="addr-body-text">${detail}</div><div class="addr-change-text">Ubah Alamat <i class="fas fa-chevron-right"></i></div>`; closeAddressModal(); }
        ...
        function renderMessagesSidebar(){
            ...
            messages.forEach(m=>{
                ...
                c.innerHTML+=`<div class="message-wrapper ${m.type}"><div class="message-bubble ${bubbleClass}" onclick="showChatOptions(event, ${m.id}, '${m.type}')">${m.text}</div><span class="message-time">${m.time}</span></div>`;
            });
        ...
        function deleteMessage(id) {
            ...
            fetch('dashboard.php', {method:'POST', body:fd}).then(res=>res.json()).then(data=>{
                if(data.status==='success'){
                    messages = messages.filter(m => m.id !== id); // Remove local
                    renderMessagesSidebar();
        ...
        const track=document.getElementById('carouselTrack');let ci=0;setInterval(()=>{ci=(ci+1)%2;track.style.transform=`translateX(-${ci*100}%)`;},5000);
    </script>
</body>
</html>